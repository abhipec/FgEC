...
if __name__ == '__main__':
    ...
    checkpoint_files = glob.glob(ckpt_directory + '/*.index')
    model_numbers = sorted([int(x[x.find('-') + 1:-6]) for x in checkpoint_files], reverse=True)
    ...
    for model_number in model_numbers[:40]:
        with tf.Graph().as_default(), tf.Session() as sess:
            ...
            output_file_path = result_directory + str(model_number)
            with open(output_file_path, 'w', encoding='utf-8') as file_p:
                ...
                time.sleep(1)
                load_checkpoint(ckpt_directory, sess, model_number)
                try:
